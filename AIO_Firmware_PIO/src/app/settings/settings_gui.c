...
void display_settings(const char *cur_ver, const char *new_ver, lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(qq_label, "");

    // if (strcmp(cur_ver, new_ver) < 0)
    if (strcmp(cur_ver, new_ver) != 0)
    {
        lv_label_set_text_fmt(new_ver_label, "Modified by #ff0000 BXCHEN #,the latest version is  #ff0000 %s#,please update. #",new_ver);
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
    else
    {
        lv_label_set_text_fmt(new_ver_label, "Modified by #ff0000 BXCHEN #,hope you like :) #");
        lv_obj_align(new_ver_label, LV_ALIGN_CENTER, 0, 60);
    }
