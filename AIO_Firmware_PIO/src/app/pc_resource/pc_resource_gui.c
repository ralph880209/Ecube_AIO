...
void display_pc_resource_init(void)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        else // 扩展组件，只使用组件的扩展属性
        {
            ...
            lv_label_set_text_fmt(net_up_label, "\uf093%d.%dKB", 12346, 5);
            ...
            lv_label_set_text_fmt(net_down_label, "\uf019%d.%dKB", 22441, 5);
...
static void extension_1_update(struct PC_Resource info)
{
    ...
    lv_label_set_text_fmt(smod[MODULE_EXT].extension_label_1, "\uf093%d.%dKB", num, mini_num);
}
...
static void extension_2_update(struct PC_Resource info)
{
    ...
    lv_label_set_text_fmt(smod[MODULE_EXT].extension_label_2, "\uf019%d.%dKB", num, mini_num);
}
