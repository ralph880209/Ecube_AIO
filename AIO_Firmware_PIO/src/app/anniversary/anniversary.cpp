...
static long long get_timestamp(String url)
{
    ...
    int time_index = payload.indexOf("unixtime") + 10; // "unixtime": 和空格的长度
    int time_end_index = payload.indexOf(",", time_index); // 查找逗号
    if (time_end_index == -1) {
    time_end_index = payload.indexOf("}", time_index); // 如果没有逗号，查找结束的花括号
    }
