...
const void *weaImage_map[] = {&weather_0, &weather_9, &weather_14, &weather_5, &weather_25,
                              &weather_30, &weather_26, &weather_11};
// 太空人图标路径的映射关系
...
static const char airQualityCh[8][10] = {"晴", "陰", "哈", "多雲", "哈", "哈","哈","雨"};
...
void display_curve_init(lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(titleLabel, "查看更多天氣");
...
void display_weather_init(lv_scr_load_anim_t anim_type)
{
    ...
    lv_label_set_text(txtLabel, "最低氣溫12°C, ");
...
void display_weather(struct Weather weaInfo, lv_scr_load_anim_t anim_type)
{
    ...
    // 下面这行代码可能会出错
    lv_label_set_text_fmt(txtLabel, "最低氣溫%d°C, 最高氣溫%d°C, 降雨機率%d%%     ",
                          weaInfo.minTemp, weaInfo.maxTemp, weaInfo.pop);
